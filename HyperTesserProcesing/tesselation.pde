class tesselation{
  ...
  tesselation(int ip,int iq,int ir,int idepth){
    ...
    cRb=cR.conj();
    elements=new ArrayList<trgroupel>();
    Path =new ArrayList<pArc>();
  }
  ...
  void populateelementssector(int i){        
    ...
    while(curword.length()<=i){
      ...
      if ((curword.equals(ta)&&((phi<=-PI+PI/p-angleToleranceToZero)||(phi>=PI-PI/p-angleToleranceToZero)))&&((ta.length()==0)||(ta.charAt(0)=='b')||(ta.charAt(0)=='d'))){elements.add(tg);}
      curword=getNextWord(curword);
    }
    println(elements.size());
  }
  
  void populateelementssectoriterative(){        
    String ta;
    trgroupel tg;
    if(curword.length()<=tessdepth){
      tg=lazytrueaddress(curword);
      ta=tg.address;
      if ((curword.equals(ta))&&((ta.length()==0)||(ta.charAt(0)=='b')||(ta.charAt(0)=='d'))){elements.add(tg);}
      curword=getNextWord(curword);
    }
  }
  ...
  trgroupel getNearestGroupel(Complex goal,int limit){
    ...
    while((goal.abs()>aP.abs()*0.8)&&i<limit){
      ...
      gab=MAb.applyinv(goal);
      gbb=MBb.applyinv(goal);
      j=min4(goal.abs(),ga.abs(),gb.abs(),gab.abs(),gbb.abs(),groupelTolerance);
      switch(j){
        case 0:
          i=limit;
          break;
  ...
  pathel getNearestPathel(Complex goal,int limit){
    ...
    int j=min4(tm.abs(),tm.sub(cP).abs(),tm.sub(cQ).abs(),tm.sub(cR).abs(),tm.sub(cRb).abs(),0.0001);
    pathel res=new pathel(tr.address,j);
    return res;
  }
